...
class Subsetter:
    ...
    def get_indices_to_keep(self, features, pairwise_matches):
        print(f"[Subsetter] Checking {len(pairwise_matches)} pairwise matches with threshold {self.confidence_threshold}")
        # Inspect matches logic
        for pm in pairwise_matches:
            if pm.src_img_idx != pm.dst_img_idx:
                print(f"[Subsetter] Match {pm.src_img_idx}->{pm.dst_img_idx}: conf={pm.confidence:.4f}, matches={len(pm.matches)}")
        ...
        print(f"[Subsetter] Indices kept: {indices}")
