...
class ProductMatcher:
    def __init__(self, config):
        ...
        config: Namespace or dict containing:
            yolo_path, device, batch_size, top_k, dino_threshold,
            lg_norm_threshold, lg_min_inliers, alpha, beta, etc.
        """
        self.cfg = config
        self.device_str = getattr(config, 'device', 'cuda' if torch.cuda.is_available() else 'cpu')
        ...
        (self.yolo_model, 
         ...
         self.sp_session, 
         self.lg_session, 
         self.device) = load_model(config.yolo_path, self.device_str)
    ...
    def process_image(self, image_path):
        ...
        # Handle YOLO Output (OBB or Standard)
        obb_polygons = []
        boxes = []
        ...
        query_dino = batch_extract_dino_features(
            crops_clahe, self.dinov3_processor, self.dinov3_model, self.device, self.cfg.batch_size
        )
            ...
            for name, items in class_groups.items():
                for idx, score in items[:2]: # Top 2 per class
                    if score >= self.cfg.dino_high_conf_threshold:
                        # High confidence DINO, skip LightGlue
                        h_score = alpha_norm * score + beta_norm * 1.0
                        if h_score > best_res['score']:
                            best_res = {'score': h_score, 'class': name, 'lg': 1.0, 
                                      'inliers': self.cfg.skip_lg_inliers_value, 'dino_score': score}
                    else:
                        if idx < len(self.support_db['sp_features']):
                            supports_to_verify.append(self.support_db['sp_features'][idx])
                            job_meta.append((name, score))

            # Batch LightGlue
            if supports_to_verify:
                lg_results = run_fast_hybrid_matching(
                    query_feat, supports_to_verify, self.lg_session, batch_size=min(self.cfg.batch_size, 32)
                )

                for k, (inliers, min_kpts) in enumerate(lg_results):
                    cls_name, d_score = job_meta[k]
                    lg_score = inliers / min_kpts if min_kpts > 0 else 0
                    h_score = alpha_norm * d_score + beta_norm * lg_score

                    if h_score > best_res['score']:
                        best_res = {'score': h_score, 'class': cls_name, 'lg': lg_score, 
                                  'inliers': inliers, 'dino_score': d_score}

            # Final Decision
            is_dino_good = any(s >= self.cfg.dino_high_conf_threshold for _, _, s in candidates)
        ...
        return img, {'matches': matches, 'classes': [m['class'] for m in matches if m['matched']], 'timing': timing_info}
